...
export default function Input(props) {
  ...
  if (props.className) className.join(props.className);
  ...
  if (props.isInvalid) {
    className.push("border-input-error");
  }
  return (
    <div className={containerClassName.join(" ")}>
      {props.label && (
        <label className={labelClassName.join(" ")}>{props.label}</label>
      )}
      <div className="w-100 position-relative ">
        <input
          ...
          type={props.typeInput ? props.typeInput : inputType}
        />
        {props.isPassword && (
      ...
      {props.isInvalid && (
        <div className={errorClassName.join(" ")}>{props.error}</div>
      )}
    </div>
  );
}
